...
OrderManager::OrderManager(CoffeeShop& shop, std::shared_ptr<LoggerObserver> logger)
: shop_(shop), logger_(logger) {}

OrderManager::~OrderManager() = default;

std::shared_ptr<Order> OrderManager::createOrder(const std::string& drinkType, const std::string& paymentType) {
    int id = nextId_.fetch_add(1);
    auto drink = DrinkFactory::create(drinkType);
    auto payment = PaymentFactory::create(paymentType);

    auto order = std::make_shared<Order>(id, std::move(drink), std::move(payment));

    // Attach logger so creation is logged when we set status.
    if (logger_) order->attach(logger_);
    ...
    // mark placed (this will notify observers / logger)
